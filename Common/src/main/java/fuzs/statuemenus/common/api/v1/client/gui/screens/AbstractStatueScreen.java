...
public abstract class AbstractStatueScreen extends Screen implements MenuAccess<StatueMenu>, StatueScreen {
        ...
        if (!this.disableMenuRendering()) {
            if (this.showInventoryEntity()) {
                ...
                this.renderArmorStandInInventory(guiGraphics,
                        ...
                        posX + this.getInventoryEntityScissorWidth(this.smallInventoryEntity) / 2.0F,
                        posY + this.getInventoryEntityScissorHeight(this.smallInventoryEntity) / 2.0F,
                        partialTick);
            }
            getHoveredTab(this.leftPos,
                    this.topPos,
                    this.imageHeight,
                    mouseX,
                    mouseY,
    ...
    public void extractBackground(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        if (!this.disableMenuRendering()) {
            ...
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED,
                    ...
                    256,
                    256);
            extractTabBackground(guiGraphics,
                    this.leftPos,
                    this.topPos,
                    this.imageHeight,
                    this,
    ...
    public static <T extends Screen & StatueScreen> boolean handleMouseScrolled(int mouseX, int mouseY, double delta, int leftPos, int topPos, int imageHeight, T screen, List<StatueScreenType> tabs) {
        ...
        if (delta != 0.0) {
            Optional<StatueScreenType> optional = getHoveredTab(leftPos, topPos, imageHeight, mouseX, mouseY, tabs);
    ...
    public static <T extends Screen & StatueScreen> boolean handleTabClicked(int mouseX, int mouseY, int leftPos, int topPos, int imageHeight, T screen, List<StatueScreenType> tabs) {
        Optional<StatueScreenType> hoveredTab = getHoveredTab(leftPos, topPos, imageHeight, mouseX, mouseY, tabs);
    ...
    public static <T extends Screen & StatueScreen> void extractTabBackground(GuiGraphicsExtractor guiGraphics, int leftPos, int topPos, int imageHeight, T screen, int mouseX, int mouseY, List<StatueScreenType> tabs) {
        ...
        for (int i = 0; i < tabs.size(); i++) {
            ...
            boolean selected = tabType == screen.getScreenType();
            Identifier sprite = sprites.get(selected, false);
            ...
            if (!selected && mouseX > tabX && mouseX <= tabX + 32 && mouseY > tabY && mouseY <= tabY + 26) {
                guiGraphics.requestCursor(CursorTypes.POINTING_HAND);
            }
        }
    }

    public static Optional<StatueScreenType> getHoveredTab(int leftPos, int topPos, int imageHeight, int mouseX, int mouseY, List<StatueScreenType> tabs) {
        int tabsStartY = getTabsStartY(imageHeight, tabs.size());
        for (int i = 0; i < tabs.size(); i++) {
            int tabX = leftPos - 32;
            int tabY = topPos + tabsStartY + 27 * i;
